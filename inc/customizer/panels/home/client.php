...
function jobscout_pro_customize_register_frontpage_client( $wp_customize ){
    ...
    $wp_customize->add_section(
        'client_section',
        array(
            'title'    => __( 'Client Section', 'jobscout-pro' ),
            'priority' => 45,
        ...
        $wp_customize->add_control(
            new JobScout_Pro_Note_Control( 
                ...
                array(
                    ...
                    'description' => sprintf( __( 'Please install/activate the %1$sRaraTheme Companion%2$s Plugin to add "Rara: Client Logo" Widget.', 'jobscout-pro' ), '<a href="' . admin_url( 'themes.php?page=tgmpa-install-plugins' ) . '" target="_blank">', '</a>' ),
                )
            )
        );
    }
    ...
    if ( ! empty( $home_client_settings ) ) {
        ...
        $wp_customize->get_section( 'sidebar-widgets-client' )->title = __( 'Client Section','jobscout-pro' );
        if( ! $raratheme_companion_activated  ) {
...
add_action( 'customize_register', 'jobscout_pro_customize_register_frontpage_client' );