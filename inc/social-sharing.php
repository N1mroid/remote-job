...
if( ! function_exists( 'jobscout_pro_opengraph_add_prefix' ) ) :
/**
 * Add Open Graph XML prefix to <html> element.
 *
 * @uses apply_filters calls 'opengraph_prefixes' filter on RDFa prefix array
 */
function jobscout_pro_opengraph_add_prefix( $output ) {
	$prefixes = array(
		'og' => 'http://ogp.me/ns#',
	);
	$prefixes = apply_filters( 'jobscout_pro_opengraph_prefixes', $prefixes );
...
add_filter( 'language_attributes', 'jobscout_pro_opengraph_add_prefix' );

/**
 * Register filters for default Open Graph metadata.
 */
function jobscout_pro_opengraph_default_metadata() {	
    if( get_theme_mod( 'ed_og_tags', true ) ){
        // additional prefixes
    	add_filter( 'jobscout_pro_opengraph_prefixes', 'jobscout_pro_opengraph_additional_prefixes' );
        
        // core metadata attributes
    	add_filter( 'jobscout_pro_opengraph_title', 'jobscout_pro_opengraph_default_title', 5 );
    	add_filter( 'jobscout_pro_opengraph_type', 'jobscout_pro_opengraph_default_type', 5 );
    	add_filter( 'jobscout_pro_opengraph_image', 'jobscout_pro_opengraph_default_image', 5 );
    	add_filter( 'jobscout_pro_opengraph_url', 'jobscout_pro_opengraph_default_url', 5 );
    
    	add_filter( 'jobscout_pro_opengraph_description', 'jobscout_pro_opengraph_default_description', 5 );
    	add_filter( 'jobscout_pro_opengraph_locale', 'jobscout_pro_opengraph_default_locale', 5 );
    	add_filter( 'jobscout_pro_opengraph_site_name', 'jobscout_pro_opengraph_default_sitename', 5 );
    
    	// additional profile metadata
    	add_filter( 'jobscout_pro_opengraph_metadata', 'jobscout_pro_opengraph_profile_metadata' );
    
    	// additional article metadata
    	add_filter( 'jobscout_pro_opengraph_metadata', 'jobscout_pro_opengraph_article_metadata' );
    
    	// twitter card metadata
    	add_filter( 'jobscout_pro_twitter_card', 'jobscout_pro_twitter_default_card', 5 );
    	add_filter( 'jobscout_pro_twitter_creator', 'jobscout_pro_twitter_default_creator', 5 );    
    }
}
add_action( 'wp', 'jobscout_pro_opengraph_default_metadata' );

if( ! function_exists( 'jobscout_pro_opengraph_additional_prefixes' ) ) :
/**
 * Add additional prefix namespaces that are supported by the opengraph plugin.
 */
function jobscout_pro_opengraph_additional_prefixes( $prefixes ) {
	if ( is_author() ) {
...
if( ! function_exists( 'jobscout_pro_opengraph_default_title' ) ) :
/**
 * Default title property, using the page title.
 */
function jobscout_pro_opengraph_default_title( $title ) {
	if ( $title ) {
		return $title;
	}
...
if( ! function_exists( 'jobscout_pro_opengraph_default_type' ) ) :
/**
 * Default type property.
 */
function jobscout_pro_opengraph_default_type( $type ) {
	if ( empty( $type ) ) {
...
if( ! function_exists( 'jobscout_pro_opengraph_default_image' ) ) :
/**
 * Default image property, using the post-thumbnail and any attached images.
 */
function jobscout_pro_opengraph_default_image( $image ) {
	if ( $image ) {
		return $image;
	}

	// As of July 2014, Facebook seems to only let you select from the first 3 images
	$max_images = apply_filters( 'jobscout_pro_opengraph_max_images', 3 );
...
if( ! function_exists( 'jobscout_pro_opengraph_default_url' ) ) :
/**
 * Default url property, using the permalink for the page.
 */
function jobscout_pro_opengraph_default_url( $url ) {
	if ( empty( $url ) ) {
		if ( is_singular() ) {
			$url = get_permalink();
		} else if ( is_author() ) {
...
if( ! function_exists( 'jobscout_pro_opengraph_default_description' ) ) :
/**
 * Default description property, using the excerpt or content for posts, or the
 * bloginfo description.
 */
function jobscout_pro_opengraph_default_description( $description ) {
	if ( $description ) {
		return $description;
	}
	...
	$description = jobscout_pro_opengraph_trim_text( $description );

	return $description;
}
endif;

if( ! function_exists( 'jobscout_pro_opengraph_default_locale' ) ) :
/**
 * Default locale property, using the WordPress locale.
 */
function jobscout_pro_opengraph_default_locale( $locale ) {
	if ( empty( $locale ) ) {
		$locale = get_locale();
	}
...
if( ! function_exists( 'jobscout_pro_opengraph_default_sitename' ) ) :
/**
 * Default site_name property, using the bloginfo name.
 */
function jobscout_pro_opengraph_default_sitename( $name ) {
	if ( empty( $name ) ) {
...
if( ! function_exists( 'jobscout_pro_opengraph_profile_metadata' ) ) :
/**
 * Include profile metadata for author pages.
 *
 * @link http://ogp.me/#type_profile
 */
function jobscout_pro_opengraph_profile_metadata( $metadata ) {
	if ( is_author() ) {
		$id = get_queried_object_id();
...
if( ! function_exists( 'jobscout_pro_opengraph_article_metadata' ) ) :
/**
 * Include article metadata for posts and pages.
 *
 * @link http://ogp.me/#type_article
 */
function jobscout_pro_opengraph_article_metadata( $metadata ) {
	if ( ! is_singular() ) {
		return $metadata;
	}
...
if( ! function_exists( 'jobscout_pro_twitter_default_card' ) ) :
/**
 * Default twitter-card type.
 */
function jobscout_pro_twitter_default_card( $card ) {
	if ( $card ) {
		return $card;
	}

	$card = 'summary';
	$images = apply_filters( 'jobscout_pro_opengraph_image', null );
...
if( ! function_exists( 'jobscout_pro_twitter_default_creator' ) ) :
/**
 * Default twitter-card creator.
 */
function jobscout_pro_twitter_default_creator( $creator ) {
	...
	$post = get_queried_object();
	$author = $post->post_author;
	$socials = get_user_meta( $author, '_jobscout_pro_user_social_icons', true );
...
if( ! function_exists( 'jobscout_pro_opengraph_metadata' ) ) :
/**
 * Get the Open Graph metadata for the current page.
 *
 * @uses apply_filters() Calls 'jobscout_pro_opengraph_{$name}' for each property name
 * @uses apply_filters() Calls 'jobscout_pro_twitter_{$name}' for each property name
 * @uses apply_filters() Calls 'jobscout_pro_opengraph_metadata' before returning metadata array
 */
function jobscout_pro_opengraph_metadata() {
	$metadata = array();
	...
	foreach ( $properties as $property ) {
		$filter = 'jobscout_pro_opengraph_' . $property;
	...
	foreach ( $twitter_properties as $property ) {
		$filter = 'jobscout_pro_twitter_' . $property;
		$metadata[ "twitter:$property" ] = apply_filters( $filter, '' );
	}

	return apply_filters( 'jobscout_pro_opengraph_metadata', $metadata );
}
endif;

if( ! function_exists( 'jobscout_pro_opengraph_site_icon_image_sizes' ) ) :
/**
 ...
 */
function jobscout_pro_opengraph_site_icon_image_sizes( $sizes ) {
	$sizes[] = 512;

	return array_unique( $sizes );
}
endif;
add_filter( 'site_icon_image_sizes', 'jobscout_pro_opengraph_site_icon_image_sizes' );

if( ! function_exists( 'jobscout_pro_opengraph_trim_text' ) ) :
/**
 * Helper function to trim text using the same default values for length and
 * 'more' text as wp_trim_excerpt.
 */
function jobscout_pro_opengraph_trim_text( $text ) {
...
if( ! function_exists( 'jobscout_pro_opengraph_meta_tags' ) ) :
/**
 * Output Open Graph <meta> tags in the page header.
 */
function jobscout_pro_opengraph_meta_tags() {
	if( get_theme_mod( 'ed_og_tags', true ) ){
        $metadata = jobscout_pro_opengraph_metadata();
...
add_action( 'wp_head', 'jobscout_pro_opengraph_meta_tags' );

if( ! function_exists( 'jobscout_pro_get_social_share' ) ) :
/**
 * Get list of social sharing icons
 * http://www.sharelinkgenerator.com/
 * 
*/
function jobscout_pro_get_social_share( $share ){
    global $post;
    ...
    switch( $share ){
        ...
        if ( $image ) {
            echo '<li><a href="' . esc_url( 'https://pinterest.com/pin/create/button/?url=' . get_the_permalink( $post->ID ) . '&media='. esc_url( $image[0] ) .'&description=' . get_the_title( $post->ID )  ) . '" rel="nofollow noopener" target="_blank" data-pin-do="none" data-pin-custom="true"><i class="fab fa-pinterest" aria-hidden="true"></i></a></li>';  
        }   
        break;
...
if( ! function_exists( 'jobscout_pro_social_share' ) ) :
/**
 * Prints Social Sharing
*/
function jobscout_pro_social_share( $floating = false ){ 
    ...
    <div class="job-share<?php echo esc_attr( $class ); ?>">
    	<?php echo '<span>'. esc_html__( 'Share on', 'jobscout-pro' ) . '</span>'; ?>
		<ul class="social-list">
			<?php
                foreach( $social_share as $share ){
                    jobscout_pro_get_social_share( $share );
                }
            ?>
		</ul>
	</div>
