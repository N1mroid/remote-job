 ...
 */
final class JobScout_Pro_Webfonts_Local{
	...
	public function add_styles() {
		...
		foreach( $hosted_fonts as $family ){
			...
			$css .= JobScout_Pro_Google_Local::init( $family )->get_css();
		}

		// If we've got CSS, add to the footer.
		if ( $css ) {
			echo '<style id="jobscout-pro-local-webfonts">' . $css . '</style>'; // WPCS: XSS ok.
		}
	}
}